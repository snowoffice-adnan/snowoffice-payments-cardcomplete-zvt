...
public class ZvtClient : IDisposable
{
    ...
    */
    ...
    /// <summary>
    /// Repeat Receipt (06 20) with CardComplete TLV selection: 1F01 = 06.
    /// CardComplete supports only tag 1F01.
    /// </summary>
    public async Task<CommandResponse> RepeatLastReceiptReconciliationAsync(CancellationToken cancellationToken = default)
    {
        this._logger.LogInformation($"{nameof(RepeatLastReceiptReconciliationAsync)} - Execute");

        var package = new List<byte>();
        package.AddRange(this._passwordData);

        // TLV container indicator (per how RegistrationAsync builds TLV blocks)
        // TLV content: Tag 1F 01, Length 01, Value 06  => 4 bytes
        package.Add(0x06); // TLV indicator
        package.Add(0x04); // TLV length
        package.Add(0x1F); // Tag byte 1
        package.Add(0x01); // Tag byte 2
        package.Add(0x01); // Value length
        package.Add(0x06); // Value: reconciliation
